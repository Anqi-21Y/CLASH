...
$res = $db->query("
    ...
    FROM respuestas
    WHERE sesion_id = $id AND es_correcta = 1
    GROUP BY jugador_id
    ORDER BY total_puntos DESC, aciertos DESC
");

$posicion = 1;
while ($row = $res->fetchArray(SQLITE3_ASSOC)) {
    $ins = $db->prepare("
        INSERT INTO resultados (sesion_id, jugador_id, puntos_total, aciertos, posicion)
        VALUES (:sid, :jid, :pts, :act, :pos)
    ");
